...
public class QuizDAO implements DAO<QuizEntity> {
    ...
    public void deleteQuiz(Long id) throws DAOException {
        Connection conn = null;
        PreparedStatement preparedStatement = null;
        ResultSet autoGeneratedValues = null;
    ...
    public void changeQuizAvailability(long id) throws DAOException {
        ...
        try {
            /* Récupération d'une connexion depuis la Factory */
            conn = daoFactory.getConnection();
            preparedStatement = initPreparedStatement(conn, SQL_CHANGE_AVAILABILITY, true, id);
            int status = preparedStatement.executeUpdate();

            if (status == 0) {
                throw new DAOException("Echec du changement de disponibilité !");
            }
        } catch (SQLException e) {
            throw new DAOException(e);
        } finally {
